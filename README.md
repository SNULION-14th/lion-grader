# SNULION 14th 챌린지 채점기 (lion-grader)

세미나 챌린지 **과제 2**의 4 서비스 결함 풀이를 자동 검증하는 CLI 채점기입니다.
*자신이 AWS에 배포한 백엔드*를 대상으로 동작합니다.
...
| 서비스 | 검증 핵심 | 채점 시간 |
|---|---|---|
| **LionPay** (C2) | 송금 동시성 — hot-sender 80건 동시 송금(총합 보존) + 잔액 부족 동시 송금 1건만 통과(행 잠금) | ~30초 |
| **LionChat** (C5) | 동기 블로킹 — 무거운 LLM 호출 12건과 가벼운 GET 50건 동시 부하 시, 가벼운 요청 p95 < 1000ms | ~1분 |
| **LionLink** (Sec14) | SSRF — IMDS·loopback·private·DNS 우회·옥타/십진/IPv6-mapped 인코딩 우회 8 vector 차단 | ~10초 |
| **LionScribe** (R1) | Durable queue — 5건 작업 제출 후 chaos restart, 90초 뒤 5/5 completed + 실제 PDF 생성 | ~2분 |

각 채점은 PASS/FAIL 외에도 **무엇을 시도했는지 · 어떤 결과가 나왔는지 · 왜 실패했는지 · 어느 방향으로 풀어야 하는지**를 친절하게 안내합니다.
...
이 박스가 보이면 통과입니다. 박스 전체를 캡처해 제출합니다.
...
- **LionScribe 채점은 백엔드를 강제 종료(chaos)시킵니다** — `systemd Restart=always`
  같은 자동 재시작 설정이 없으면 백엔드가 다시 안 살아납니다. 채점기가 자동으로
  health check를 시도하니, 20초 안에 복구되지 않으면 FAIL이 떨어집니다.

- **HTTPS 필수가 아닙니다** — HTTP URL도 지원. 단, 학생 배포가 HTTPS면 인증서가
  유효해야 합니다 (Let's Encrypt 등). 자체 서명 인증서는 실패합니다.
...
**과제 PASS 결과 박스를 스크린샷으로 캡처해서 제출**합니다. 4 서비스 각각 따로 PASS 필요.

제출 형식·기한 등 상세는 챌린지 공지를 따르세요.
